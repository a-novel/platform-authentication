<script lang="ts">
  ...
  // Props.
  // ===================================================================================================================
  type FormProps = ComponentProps<typeof RegisterPage>;
  ...
  // Stores & dependencies.
  // ===================================================================================================================
  const { t } = getTranslate("auth.page.register");
  ...
  async function onsubmit(evt: Parameters<EventHandler<SubmitEvent, HTMLFormElement>>[0]) {
    ...
    const formData = await ShortCodeCreateRegisterRequestSchema.safeParseAsync(rawFormData);
    ...
    // Submit to API.
    await shortCodeCreateRegister(api, session.accessToken, formData.data)
      .then(() => {
  ...
  function setEmail(value: string) {
    ...
    checkEmailDebouncer.call(() => {
      validators.validateEmail(value, api, setEmailStatus, true);
    });
  }
</script>
